...
extern  string    __LSMAStrategySetting = "------2 LSMA Strategy Setting------";
extern    int     Lsma_Period_Entry = 25;
extern    int     Lsma_Period_Trend = 50;
...
public :
	C2LSMAStrategy(string symbol){
		...
		_lsmaEntry      = new CLSMA(_symbol,Lsma_TF_Middle);
    _lsmaTrend.SetPeriod(Lsma_Period_Trend);
    _lsmaEntry.SetPeriod(Lsma_Period_Entry);
		_signal         = new CSignal();
		_zigZag         = new CZigZag();
		_indicatorCount = 0;
		ArrayResize( _indicators, 5 );
		
		_indicators[_indicatorCount] = new CIndicator("LSMA_50" );
      _indicatorCount++;
	
      _indicators[_indicatorCount] = new CIndicator("LSMA_25");
      _indicatorCount++;
    ...
    // clear indicators
      ...
      if(trend==TREND_LONG){
         _signal.IsBuy = true;
         _signal.StopLoss = iLow(_symbol,Lsma_TF_Middle,barShift);
         ...
         _signal.IsSell = true;
         _signal.StopLoss = iHigh(_symbol,Lsma_TF_Middle,barShift);
