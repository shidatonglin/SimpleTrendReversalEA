...
extern string      ChartTemplate                = "HeiKen.tpl";
...
#include <LSMA\CHeiKenAshiStrategy.mqh>
...
int OnInit()
{
   ...
   {
      _pairs[0]  = new CPair(Symbol(), new CHeiKenAshiStrategy(Symbol()), _newsFilter, _utils);
      _pairCount = 1;
   }
   else
   {
      ...
      for (int i=0; i <  SymbolsTotal(true); ++i)
      {
         ...
         for (int x=0; x < pairCount; ++x)
         {
            ...
            {
               _pairs[_pairCount] = new CPair(symbol, new CHeiKenAshiStrategy(symbol), _newsFilter, _utils);
               _pairCount++;
               break;
            }
         }
