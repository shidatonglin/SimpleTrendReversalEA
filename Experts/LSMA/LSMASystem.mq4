...
#include <LSMA\CLSMAStrategy.mqh>
...
int OnInit()
{
   ...
   {
      _pairs[0]  = new CPair(Symbol(), new CLSMAStrategy(Symbol()), _newsFilter, _utils);
      _pairCount = 1;
   }
   else
   {
      ...
      for (int i=0; i <  SymbolsTotal(true); ++i)
      {
         ...
         for (int x=0; x < pairCount; ++x)
         {
            ...
            {
               _pairs[_pairCount] = new CPair(symbol, new CLSMAStrategy(symbol), _newsFilter, _utils);
               _pairCount++;
               break;
            }
         }
