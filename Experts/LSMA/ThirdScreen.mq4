...
extern string      ChartTemplate                = "ThirdScreen.tpl";
...
#include <LSMA\CThreeScreenStrategy.mqh>
...
{   
   ...
   _labelExpectedPayOff.Text = DoubleToStr(_tradeStats.ExpectedPayOff, 2)+ " " + AccountCurrency() + " / trade";
...
int OnInit()
{
   ...
   {
      _pairs[0]  = new CPair(Symbol(), new CThreeScreenStrategy(Symbol()), _newsFilter, _utils);
      _pairCount = 1;
   }
   else
   {
      ...
      for (int i=0; i <  SymbolsTotal(true); ++i)
      {
         ...
         for (int x=0; x < pairCount; ++x)
         {
            ...
            {
               _pairs[_pairCount] = new CPair(symbol, new CThreeScreenStrategy(symbol), _newsFilter, _utils);
               _pairCount++;
               break;
            }
         }
